"""
benchmark.py  —  the 10 questions you run against BOTH toolsets.

How to use it:
  1. Run all 10 questions against Toolset A (the rough one) with your Week 1
     agent, 3 times each. Record the four metrics (see SCORES below).
  2. Design and build Toolset B (spec reviewed with your mentor FIRST).
  3. Run the same 10 against Toolset B, 3 times each.
  4. Fill in the comparison table in the Week 2 doc and explain the deltas.

Each question carries the ground truth so you can score "correctness"
objectively. `accept` lists strings, ANY of which appearing in the final
answer counts as correct (handles "4,420" vs "4420" etc.). `must_not_say`
catches a common wrong answer where useful.

The categories matter: the `arithmetic` ones (B3, B8) are where Toolset A's
"the model adds it up itself" tends to go wrong, and where Toolset B's
dedicated sum tool should make errors structurally impossible. B10 is a trap:
the data/tools cannot answer it, so the ONLY correct behaviour is to say so —
...
# ---------------------------------------------------------------------------
def run_benchmark(run_agent, tools, tool_impls, runs: int = 3):
    """
    ...
    for case in BENCHMARK:
        ...
        total_correct = 0
        for _ in range(runs):
            out = run_agent(case["question"], tools, tool_impls)
            per_run.append({
...
if __name__ == "__main__":
    ...
    from loop_W2_toolB import run_agent
    run_benchmark(run_agent, TOOLS_B, TOOLS_B_IMPLS)

    # from toolset_a import TOOLS_A, TOOLS_A_IMPLS
    #from loop_W2_toolA import run_agent
    # run_benchmark(run_agent, TOOLS_A, TOOLS_A_IMPLS)